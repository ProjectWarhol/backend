...
const getEth = async (address) => await web3.eth.getBalance(address) / 1000000000000000000
...
contract('PostNftMinting', accounts => {
	...
	beforeEach(async () => {
		...
		acc1 = accounts[1]
		console.log('\n-----')
		console.log(`Acc0 balance: ${await getEth(acc0)}`)
		console.log(`Acc1 balance: ${await getEth(acc1)}`)
	})
	...
	it('Minting should fail due to invalid address', async () => {
		try {
			...
			assert.strictEqual(e.code, 'INVALID_ARGUMENT', 'Receiver address is not valid')
		}
	})
	...
	it('URI of token is correct', async () => {
		...
		tokenCounter += 1
		assert.strictEqual(await instance.tokenURI(tokenCounter), URI, 'The URI should be correct')
	})

	it('Owner of token should be receiver', async () => {		
		await instance.safeMint(acc1, URI)
		tokenCounter += 1
		assert.strictEqual(await instance.ownerOf(tokenCounter), acc1, 'Owner of token should be receiver')
	})
	...
	it('Token should be in receivers wallet', async () => {
		...
		assert.isAbove(amountTokensAfter, amountTokensBefore, 'Amount of tokens should be higher than before minting')
	})
	...
	it('Transfer of token should fail due to wrong ownership', async () => {
		...
		try {
			...
			assert.strictEqual(e.reason, 'ERC721: transfer caller is not owner nor approved', 'Caller is not the owner of token')
		}
	})
	...
	it('Transfer of token should fail due to non-existient token', async () => {
		...
		try {
			...
			assert.strictEqual(e.reason, 'ERC721: operator query for nonexistent token', 'Token does not exist')
		}
	})
	...
	it('Transfer of token should fail due to receiver address is not valid', async () => {
		...
		try {
			...
			assert.strictEqual(e.code, 'INVALID_ARGUMENT', 'Receover address is not valid')
		}
	})
})
